 ...
 * @description 合同台账单-》合同台账 签约方甲方名称 集成值转换
 */
public class ErContractbillSignANameValueConversionRule implements JavaValueConversionRule {
    @Override
    public Object cast(String s, ConnectionWrapper connectionWrapper, ConnectionWrapper connectionWrapper1) {
        ...
        DynamicObject object = loadSingle.getDynamicObjectCollection("contractpartyentry").stream()
                .filter(dynamicObject -> "0".equals(dynamicObject.getString("signcontract")))
                .findFirst().orElse(null);

        if(object != null){
            if(object.getDynamicObject("contractparty") != null){
                return object.getDynamicObject("contractparty").getString("name");
            }
        }
