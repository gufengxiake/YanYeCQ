...
public class StorageSummaryReportPlugin extends AbstractReportListDataPlugin {
    ...
    public DataSet query(ReportQueryParam reportQueryParam, Object o) throws Throwable {
        ...
        DataSet costrecord = QueryServiceHelper.queryDataSet(this.getClass().getName(),
                "cal_costrecord", "id,bookdate,storageorgunit,calbilltype,adminorg,entry,entry.stocktype.id,entry.id entrypk,entry.material.number as nckd_materialnum,entry.material nckd_material, entry.material.id,entry.material.modelnum as nckd_model" +
        ...
        costrecord = costrecord.groupBy(new String[]{"id","storageorgunit","entry","entrypk","nckd_materialnum", "nckd_material", "nckd_model", "nckd_assist", "nckd_lot","nckd_configuredcode","nckd_tracknumber","nckd_ispresent","nckd_isrework","nckd_baseunit","nckd_unitstandardcost","nckd_amount"}).sum("nckd_amount").sum("nckd_baseqty").finish();
        ...
        // 核算成本记录与成本调整单关联
        DataSet resultDataset = costrecord.leftJoin(calCostadjustbill).on("nckd_material", "masterid").on("id", "invbillid").on("entrypk", "invbillentryid")
                .select(new String[]{"id","entry", "entrypk","nckd_materialnum", "nckd_material", "nckd_model", "nckd_assist", "nckd_lot","nckd_configuredcode","nckd_tracknumber","nckd_ispresent","nckd_isrework","nckd_baseunit","nckd_baseqty","nckd_unitstandardcost","nckd_amount"},
                        new String[]{"( CASE WHEN adjustamt IS NULL THEN 0 ELSE adjustamt END )nckd_differencamount"}).finish();
        resultDataset = resultDataset.select("entry", "nckd_materialnum", "nckd_material", "nckd_model", "nckd_assist", "nckd_lot","nckd_configuredcode","nckd_tracknumber","nckd_ispresent","nckd_isrework","nckd_baseunit","nckd_baseqty","nckd_unitstandardcost","(nckd_differencamount + nckd_amount)nckd_amount","nckd_differencamount");
        resultDataset = resultDataset.groupBy(new String[]{"nckd_materialnum", "nckd_material", "nckd_model", "nckd_assist", "nckd_lot","nckd_configuredcode","nckd_tracknumber","nckd_ispresent","nckd_isrework","nckd_baseunit","nckd_unitstandardcost"}).sum("nckd_baseqty").sum("nckd_amount").sum("nckd_differencamount").finish();
