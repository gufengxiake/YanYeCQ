...
/**
 * 核心人力云-》人员信息-》附表弹框-》系统外任职经历（nckd_hspm_emporgrelout_dg）
 * 动态表单插件
 * author:程超华 2024-07-12
 */
public class EmpEmpOutCardEditPlugin extends AbstractFormDrawEdit {

    public EmpEmpOutCardEditPlugin() {
    ...
    public void click(EventObject evt) {
        ...
        if (key.equals("btnsave")) {
            if (status.equals(OperationStatus.EDIT)) {
                // 基础页面
                this.updateAttachData("nckd_hrpi_emporgrelout", this.getView(), true, (String)null);
            } else if (status.equals(OperationStatus.ADDNEW)) {
                this.addAttachData("nckd_hrpi_emporgrelout", this.getView(), this.getModel().getDataEntity(), true);
            }
        }
