 ...
 * author：程超华
 */
public class EmyearkaoheSaveOp extends InfoclassifySaveOp {
    private static final Log LOGGER = LogFactory.getLog(EmyearkaoheSaveOp.class);
    private final IEmyearkaoheService empproexpService = IEmyearkaoheService.getInstance();
    private final Map<Long, HrpiServiceOperateResult> operateResultMap = new HashMap(16);

    public EmyearkaoheSaveOp() {
    }
    ...
    protected void afterDelete(AfterOperationArgs args, DynamicObject[] dataEntities) {
        ...
        if (!CollectionUtils.isEmpty(successPkIds)) {
            ...
            // HR中台服务云->员工信息中心->人员附表  nckd_hrpi_yearkaohe 年度考核信息基础页面
            this.empproexpService.removeAttachment("nckd_hrpi_yearkaohe", pkIdList);
        }
