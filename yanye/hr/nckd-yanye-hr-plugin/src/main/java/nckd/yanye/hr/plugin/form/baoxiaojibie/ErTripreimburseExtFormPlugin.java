 ...
 * @description 差旅报销单-表单插件-自动获取报销级别
 */
public class ErTripreimburseExtFormPlugin extends AbstractBillPlugIn {
    @Override
    ...
    public Object getReimburse(DynamicObject reimbursesettings,DynamicObject company) {
        ...
        if (!orgEntrys.isEmpty()) {
            LinkedList<ErTripreimburseExtFormPlugin.OrgInfo> orgs = this.getOrgs(orgEntrys);
            List<ErTripreimburseExtFormPlugin.OrgInfo> companys = this.getCompanys(orgs);
            if (!companys.isEmpty()) {
    ...
    private List<ErTripreimburseExtFormPlugin.OrgInfo> getCompanys(LinkedList<ErTripreimburseExtFormPlugin.OrgInfo> orgs) {
        List<ErTripreimburseExtFormPlugin.OrgInfo> companys = new ArrayList(orgs.size());
        orgs.forEach((org) -> {
            ...
            ErTripreimburseExtFormPlugin.OrgInfo orgInfo = new ErTripreimburseExtFormPlugin.OrgInfo(companyId, org.ispartjob());
    ...
    private LinkedList<ErTripreimburseExtFormPlugin.OrgInfo> getOrgs(DynamicObjectCollection orgEntrys) {
        LinkedList<ErTripreimburseExtFormPlugin.OrgInfo> orgs = new LinkedList();
        orgEntrys.forEach((entry) -> {
            boolean ispartJob = entry.getBoolean("ispartjob");
            ErTripreimburseExtFormPlugin.OrgInfo info = new ErTripreimburseExtFormPlugin.OrgInfo(entry.getLong("dpt.id"), ispartJob);
            if (!ispartJob) {
                orgs.addFirst(info);
            } else {
                orgs.add(info);
    ...
    private static class OrgInfo {
        ...
        public boolean equals(Object obj) {
            if (obj == null) {
                ...
                return this.getClass() != obj.getClass() ? false : ((ErTripreimburseExtFormPlugin.OrgInfo) obj).getOrgId().equals(this.getOrgId());
            }
        }
