...
public class AbstractDesensitizeFieldCollectPluginEx extends AbstractDesensitizeFieldCommonPlugin {
    ...
    public void beforeBindData(EventObject e) {
        Map<String, Long> fieldForIdMap = fieldForIdMap();
        IDataModel model = this.getModel();

        // 手机号
        Long phoneId = fieldForIdMap.get("phone");
        String phone = "field" + phoneId;
        String phoneValue = (String) model.getValue(phone);
        // 根据手机号查入职单
        DynamicObject obj = BusinessDataServiceHelper.loadSingle(
                "hom_onbrdbillbase",
                new QFilter[]{
                        // 手机号
                        new QFilter("phone", QCP.equals, phoneValue),
                        // 入职状态：2：入职流程中
                        new QFilter("enrollstatus", QCP.equals, "2")
                }
        );
        // 招聘来源 field2049288120897125376
        Long recruitTypeId = fieldForIdMap.get("nckd_recruittype");
        String recruitType = "field" + recruitTypeId;
        if (!"fieldnull".equals(recruitType)) {
            // 赋值招聘来源
