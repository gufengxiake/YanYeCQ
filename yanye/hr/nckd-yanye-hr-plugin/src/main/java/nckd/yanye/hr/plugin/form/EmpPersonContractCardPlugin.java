...
/**
 * 核心人力云-》人员信息-》附表卡片-》人员合同信息（nckd_hspm_hrpi_hetong_dv）
 * 动态表单插件
 * author:程超华 2024-04-11
 */
public class EmpPersonContractCardPlugin extends AbstractCardDrawEdit {
    private static final Log logger = LogFactory.getLog(EmpPersonContractCardPlugin.class);

    public EmpPersonContractCardPlugin() {
    }

    // 唯一标识：取元数据nckd_hspm_hrpi_hetong_dv下内容控件contentaphetong里contentap后面的字符hetong
    @Override
    ...
    protected PreBindDataVo prefixHandlerBeforeBindData(EventObject args) {
        ...
        if (personId != null && personId != 0L) {
            // 头部：合同编号 nckd_hetongcode ，标题：签订类型 nckd_qiandingtype ，合同期限类型 nckd_hetongqixiantype，具体业务字段：
            CardViewCompareVo compareVo = new CardViewCompareVo("nckd_hetongcode", "nckd_qiandingtype",
                    "nckd_hetongqixiantype,nckd_hetongqixianmonth,nckd_startdate,nckd_enddate,nckd_shiyongstartdate,nckd_shiyongenddate,nckd_datenckd_address,nckd_hetongtype,nckd_probationmonth,nckd_economiccomp,nckd_liquidateddamages,nckd_dateauthenti,nckd_businesscompany,nckd_subjectcontract,nckd_remark");
            ...
            // 人员合同信息基础页面 页面编码：nckd_hrpi_hetong，字段：起始日期 nckd_startdate，终止日期 nckd_enddate，创建时间 createtime
            QueryDbVo queryDbVo = new QueryDbVo(conFilter, fields, "nckd_hrpi_hetong", "nckd_startdate desc,nckd_enddate desc");
    ...
    protected void customChangeLabelStyle(AfterCreatVo afterCreatVo) {
        ...
        if ("text".equals(labType)) {
            ...
            if ("nckd_qiandingtype".equals(field)) {
