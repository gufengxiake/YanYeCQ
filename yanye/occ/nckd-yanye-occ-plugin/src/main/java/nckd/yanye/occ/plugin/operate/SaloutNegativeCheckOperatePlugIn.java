...
public class SaloutNegativeCheckOperatePlugIn extends AbstractOperationServicePlugIn {
    ...
    public void onPreparePropertys(PreparePropertysEventArgs e) {
        e.getFieldKeys().add(SalOutValidator.KEY_CONFIGUREDCODE);
        e.getFieldKeys().add(SalOutValidator.KEY_OUTINVTYPE);
        e.getFieldKeys().add(SalOutValidator.KEY_AUXPTY);
        e.getFieldKeys().add(SalOutValidator.KEY_PRODUCEDATE);
        e.getFieldKeys().add(SalOutValidator.KEY_BIZOPERATOR);
        e.getFieldKeys().add(SalOutValidator.KEY_WAREHOUSE);
        e.getFieldKeys().add(SalOutValidator.KEY_MATERIAL);
        e.getFieldKeys().add(SalOutValidator.KEY_PROJECT);
        e.getFieldKeys().add(SalOutValidator.KEY_OUTOWNER);
        e.getFieldKeys().add(SalOutValidator.KEY_UNIT2ND);
        e.getFieldKeys().add(SalOutValidator.KEY_UNIT);
        e.getFieldKeys().add(SalOutValidator.KEY_WAREHOUSE);
        e.getFieldKeys().add(SalOutValidator.KEY_LOCATION);
        e.getFieldKeys().add(SalOutValidator.KEY_ORG);
        e.getFieldKeys().add(SalOutValidator.KEY_EXPIRYDATE);
        e.getFieldKeys().add(SalOutValidator.KEY_TRACKNUMBER);
        e.getFieldKeys().add(SalOutValidator.KEY_OUTKEEPER);
        e.getFieldKeys().add(SalOutValidator.KEY_BASEUNIT);
        e.getFieldKeys().add(SalOutValidator.KEY_OUTOWNERTYPE);
        e.getFieldKeys().add(SalOutValidator.KEY_OUTINVSTATUS);
        e.getFieldKeys().add(SalOutValidator.KEY_LOTNUMBER);
        e.getFieldKeys().add(SalOutValidator.KEY_OUTKEEPERTYPE);
    ...
    public void onAddValidators(AddValidatorsEventArgs e) {
        ...
        // 添加自定义的校验器
        e.addValidator(new SalOutValidator());
    }
     ...
     * @author
     */
    class SalOutValidator extends AbstractValidator {

        //余额表维度映射字段
        public final static String KEY_ORG = "org";//销售组织
        ...
        public final static String KEY_BIZOPERATOR = "bizoperator";//销售员
        ...
        public final static String KEY_WAREHOUSE = "warehouse";//仓库
        ...
        public final static String KEY_LOCATION = "location";//出库仓位
        ...
        public void validate() {
            ...
            for (ExtendedDataEntity rowDataEntity : this.getDataEntities()) {
                ...
                if ("车销出库单".equals(name) && !isvirtualbill) {
                    //单据编号
                    String billno = rowDataModel.getValue("billno").toString();

                    //表单标识
                    String number = "im_saloutbill";//销售出库单
                    //查询字段
                    String fieldkey = "org.id orgid,bizoperator.id operatorid,billentry.expirydate expirydate,billentry.tracknumber tracknumber,billentry.outkeeper.id outkeeperid,billentry.baseunit.id baseunitid,billentry.outinvtype.id outinvtypeid," +
                            ...
                            "billentry.warehouse.id warehouseid,billentry.unit2nd.id unit2ndid,billentry.auxpty.id auxptyid,billentry.project.id projectid,billentry.outowner.id outownerid,billentry.producedate producedate,billentry.location.id locationid,billentry.baseqty baseqty";
                    //过滤条件
