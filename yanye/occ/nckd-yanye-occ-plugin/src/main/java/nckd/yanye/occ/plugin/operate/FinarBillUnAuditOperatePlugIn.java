...
//                                            recRow.set("nckd_settlekamt", 0);//收款处理结算金额扣减
//                                            recRow.set("nckd_settleklocalamt", 0);//收款处理结算金额扣减本位币
//                                            recRow.set("nckd_lockekamt", 0);//收款处理锁定金额扣减
//                                        }
    ...
    public void endOperationTransaction(EndOperationTransactionArgs e) {
        ...
        if (deliverRecords != null) {
            for (DynamicObject dataObject : deliverRecords) {
                DynamicObjectCollection entry = dataObject.getDynamicObjectCollection("entry");
                if (entry != null) {
                    HashSet<Object> corebillentryidList=new HashSet<>();
                    for (DynamicObject entryRow : entry) {
                        //核心单据行Id
                        Object corebillentryid = entryRow.get("corebillentryid");
                    ...
                    //根据核心单据行Id 查找暂估应付单记录
                    // 构造QFilter
                    QFilter qFilter = new QFilter("entry.corebillentryid", QCP.in, corebillentryidList);
                    DynamicObjectCollection collections = QueryServiceHelper.query("ap_busbill",
                            "id", qFilter.toArray(), "");
                    if (!collections.isEmpty()) {
                        List<DynamicObject> updateData = new ArrayList<>();
                        for(DynamicObject busDataObject:collections){
                            Object pkId=busDataObject.get("id");
                            DynamicObject busbill = BusinessDataServiceHelper.loadSingle(pkId, "ap_busbill");
                            DynamicObjectCollection busEntry = busbill.getDynamicObjectCollection("entry");
                            for(DynamicObject entryRow:busEntry){
                                entryRow.set("nckd_finarbillaudit",false);
                            }
                            updateData.add(busbill);
